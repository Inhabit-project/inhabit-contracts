...
import './security/RoleManager.sol';
	...
	Group
{
	...
	function _buyNFT(address _collection, uint256 _amount) internal {
		require(_amount > 0, 'Amount must be greater than zero');
		CollectionStruct storage collection = collections[
			collectionIndex[_collection].index
		];
	...
	function refundInvestment(
		...
		require(refundEnabled[_msgSender()], 'Refund not enabled for this wallet');
		...
		if (_token == address(0)) {
			// Manejo explícito para token nativo (ETH)
			...
			// Lógica existente para tokens ERC20
	...
	function reserveNFTs(
		...
		for (uint256 i = 0; i < collectionCount; i++) {
			if (
				...
				keccak256(bytes(abi.encodePacked(i)))
			) {
				collectionIndex = i;
				found = true;
				break;
	...
	function setCollectionStatus(
		...
		CollectionIndexStruct storage collectionIdx = collectionIndex[
			_collectionAddr
		];
