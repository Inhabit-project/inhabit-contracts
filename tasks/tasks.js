...
task("whiteList", "show WhiteList")
    .setAction(async (taskArguments, hre, runSuper) => {
        const contractInstance = await hre.ethers.getContractAt("VendorV2", SMART_CONTRACT);
...
task("addAdmin", "Adds a Admin")
    ...
    .setAction(async (taskArguments, hre, runSuper) => {
        const { addr = "0x2Ff876F9E71924564Af7C51739E2E9A17aAa7aC0" } = taskArguments;
        const contractInstance = await hre.ethers.getContractAt("VendorV2", SMART_CONTRACT);
...
task("storeTokens", "Stores the whitelist in the contract")
    .setAction(async (taskArguments, hre, runSuper) => {
...
subtask("storeWL", "Stores the whitelist in the contract")
    ...
    .addParam("erc20", "The address of the erc20 contract", undefined, types.string)
    .addParam("oracle", "The address of the oracle contract", undefined, types.string)
    .addParam("oracleDecimals", "The decimals of the oracle contract", 10, types.int)
    ...
    .setAction(async (taskArguments, hre, runSuper) => {
        const { contract, erc20, oracle, oracleDecimals, active, isNative } = taskArguments;

        console.log({ contract, erc20, oracle, oracleDecimals, active, isNative });

        const contractInstance = await hre.ethers.getContractAt("VendorV2", contract);
        const [deployer] = await hre.ethers.getSigners();
        const feeTx = await deployer.getGasPrice();
        console.log('feeTx', feeTx.toHexString());

        const tx = await contractInstance
            .addToken(erc20, oracle, oracleDecimals, active, isNative);
...
task("collectionList", "show Collection List")
.setAction(async (taskArguments, hre, runSuper) => {
    const contractInstance = await hre.ethers.getContractAt("VendorV2", SMART_CONTRACT);
 ...
 */
task("storeCollections", "Stores the collections in the contract")
    .setAction(async (taskArguments, hre, runSuper) => {
        ...
        for (const collection of collections) {
            await hre.run("storeClltn", {
                ...
                price: hre.ethers.utils.parseUnits(collection.price.toString(), "ether").toString(),
...
subtask("storeClltn", "Stores the collections in the contract")
    ...
    .addParam("price", "The price of the collection", "1000000000000000000", types.string)
    .addParam("active", "The active status of the collection", true, types.boolean)
    .setAction(async (taskArguments, hre, runSuper) => {
