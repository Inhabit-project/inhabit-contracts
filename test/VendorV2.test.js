...
describe("VendorV2", function () {
    ...
    beforeEach(async function () {
        ...
        await vendor.connect(owner).addGroup("default", true, defaultGroup.arrayShared);
        console.log("Added default group in VendorV2 contract");

        // Configurar la colección con un precio más bajo para las pruebas
        await vendor.connect(owner).addCollection(await mockNFT.getAddress(), ethers.parseEther("0.1"), true);
    ...
    describe("Funciones básicas", function () {
        describe("Compra con tokens ERC20", function () {
            it("Debería permitir comprar NFTs con tokens ERC20", async function () {
                ...
                expect(await mockNFT.balanceOf(user1.address)).to.equal(1, "El usuario debería tener 1 NFT después de la compra");
            });
            ...
            it("Debería permitir compras múltiples de NFTs con tokens ERC20", async function () {
                ...
                expect(await mockNFT.balanceOf(user1.address)).to.equal(3, "El usuario debería tener 3 NFTs después de la compra múltiple");
            });

            it("Debería fallar la compra con tokens ERC20 si no hay suficiente balance", async function () {
                const vendorAddress = await vendor.getAddress();
                await mockToken.connect(user2).approve(vendorAddress, ethers.parseEther("100"));
                
                await expect(
                    vendor.connect(user2).buyWithToken(
                        "default",
                        await mockToken.getAddress(),
                        0,
                        1
        ...
        describe("Compra con ETH (Token Nativo)", function () {
            it("Debería permitir comprar NFTs con ETH", async function () {
                ...
                expect(await mockNFT.balanceOf(user1.address)).to.equal(1, "El usuario debería tener 1 NFT después de la compra con ETH");
            });
            ...
            it("Debería permitir compras múltiples de NFTs con ETH", async function () {
                ...
                expect(await mockNFT.balanceOf(user1.address)).to.equal(3, "El usuario debería tener 3 NFTs después de la compra múltiple con ETH");
            });
        ...
        it("Debería permitir al owner transferir NFTs reservados a un usuario", async function () {
            ...
            expect(await mockNFT.balanceOf(user1.address)).to.equal(1, "El usuario debería tener 1 NFT después de la transferencia reservada");
        });
    });
    ...
    describe("Funciones de Reembolso", function () {
        describe("Reembolso con Tokens ERC20", function () {
            it("Debería permitir reembolsar una inversión realizada con tokens ERC20", async function () {
                ...
                await mockToken.connect(owner).transfer(await vendor.getAddress(), ethers.parseEther("100"));
                ...
                expect(balanceFinal).to.be.gt(balanceInicial, "El balance del usuario debería ser mayor después del reembolso");
            });
            ...
            it("No debería permitir reembolsos de tokens ERC20 sin aprobación previa del NFT", async function () {
                ...
                await expect(
                    vendor.connect(user1).refundInvestment(
                        0,
                        await mockToken.getAddress(),
                        0
                    )
        ...
        describe("Reembolso con ETH (Token Nativo)", function () {
            it("Debería permitir reembolsar una inversión realizada con ETH", async function () {
                ...
                expect(balanceFinal).to.be.gt(balanceInicial, "El balance de ETH del usuario debería ser mayor después del reembolso");
            });
        });
    });
    ...
    describe("Controles de Seguridad", function () {
        ...
        it("No debería permitir comprar NFTs (ni con token ni con ETH) de una colección inactiva", async function () {
            ...
            await vendor.connect(owner).setCollectionStatus(await mockNFT.getAddress(), false);

            const mockTokenAddress = await mockToken.getAddress();
            await mockToken.connect(user1).approve(await vendor.getAddress(), ethers.parseEther("100"));
            ...
            await vendor.connect(owner).setCollectionStatus(await mockNFT.getAddress(), true);
        });
    });
