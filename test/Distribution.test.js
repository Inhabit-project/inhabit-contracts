...
describe("Sistema de Distribución de Comisiones", function () {
    ...
    beforeEach(async function () {
        ...
        [owner, user1, distributor1, distributor2, distributor3] = await ethers.getSigners();
        ...
        console.log("MockChainlink desplegado en:", await mockChainlink.getAddress());
        ...
        await vendor.connect(owner).addGroup("distribution70_30", true, distributionGroup70_30.arrayShared);
        await vendor.connect(owner).addGroup("distribution33_33_33", true, distributionGroup33_33_33.arrayShared);
    ...
    describe("Gestión de Grupos de Distribución", function () {
        ...
        it("Debería fallar la compra si se intenta distribuir a un grupo desactivado", async function () {
            ...
            await mockToken.connect(user1).approve(vendorAddress, ethers.parseEther("100"));
            
            await expect(
                vendor.connect(user1).buyWithToken(
                    "distribution70_30",
                    await mockToken.getAddress(),
                    0,
                    1
    ...
    describe("Distribución de Comisiones con Token ERC20", function () {
        it("Debería distribuir correctamente las comisiones 70/30 al comprar con tokens ERC20", async function () {
            ...
            const dist1BalanceInicial = await mockToken.balanceOf(distributor1.address);
            const dist2BalanceInicial = await mockToken.balanceOf(distributor2.address);

            // Aprobar tokens para la compra
            await mockToken.connect(user1).approve(vendorAddress, ethers.parseEther("100"));

            // Realizar compra
            await vendor.connect(user1).buyWithToken(
                "distribution70_30",
                await mockToken.getAddress(),
                0,
                1
            ...
            expect(dist1Ganancia).to.be.gt(0n, "El distribuidor 1 debería haber recibido tokens");
            expect(dist2Ganancia).to.be.gt(0n, "El distribuidor 2 debería haber recibido tokens");
            const ratio = (dist1Ganancia * 3n) / (dist2Ganancia * 7n);
            expect(ratio).to.be.closeTo(1n, 2n, "La proporción de distribución debería ser aproximadamente 70/30");
        });
        ...
        it("Debería distribuir correctamente las comisiones 33/33/33 al comprar con tokens ERC20", async function () {
            ...
            const dist1BalanceInicial = await mockToken.balanceOf(distributor1.address);
            const dist2BalanceInicial = await mockToken.balanceOf(distributor2.address);
            const dist3BalanceInicial = await mockToken.balanceOf(distributor3.address);
            ...
            expect(dist1Ganancia).to.be.gt(0n, "El distribuidor 1 debería haber recibido tokens");
            expect(dist2Ganancia).to.be.gt(0n, "El distribuidor 2 debería haber recibido tokens");
            expect(dist3Ganancia).to.be.gt(0n, "El distribuidor 3 debería haber recibido tokens");
            ...
            expect(Math.abs(Number(dist1Ganancia - dist2Ganancia))).to.be.lessThan(Number(maxDiferencia), "La diferencia entre distribuidor 1 y 2 debería ser mínima");
            expect(Math.abs(Number(dist2Ganancia - dist3Ganancia))).to.be.lessThan(Number(maxDiferencia), "La diferencia entre distribuidor 2 y 3 debería ser mínima");
            expect(Math.abs(Number(dist1Ganancia - dist3Ganancia))).to.be.lessThan(Number(maxDiferencia), "La diferencia entre distribuidor 1 y 3 debería ser mínima");
        });
    });
    ...
    describe("Distribución de Comisiones con Token Nativo (ETH)", function () {
        it("Debería distribuir correctamente las comisiones 70/30 al comprar con ETH", async function () {
            // Balance inicial de los distribuidores
            const dist1BalanceInicial = await ethers.provider.getBalance(distributor1.address);
            const dist2BalanceInicial = await ethers.provider.getBalance(distributor2.address);
            ...
            // Verificar distribución
            const dist1BalanceFinal = await ethers.provider.getBalance(distributor1.address);
            const dist2BalanceFinal = await ethers.provider.getBalance(distributor2.address);
            ...
            // Verificar proporciones (70/30)
            expect(dist1Ganancia).to.be.gt(0n, "El distribuidor 1 debería haber recibido ETH");
            expect(dist2Ganancia).to.be.gt(0n, "El distribuidor 2 debería haber recibido ETH");
            const ratio = (dist1Ganancia * 3n) / (dist2Ganancia * 7n);
            expect(ratio).to.be.closeTo(1n, 2n, "La proporción de distribución debería ser aproximadamente 70/30");
        });
        ...
        it("Debería distribuir correctamente las comisiones 33/33/33 al comprar con ETH", async function () {
            ...
            const dist1BalanceInicial = await ethers.provider.getBalance(distributor1.address);
            const dist2BalanceInicial = await ethers.provider.getBalance(distributor2.address);
            const dist3BalanceInicial = await ethers.provider.getBalance(distributor3.address);
            ...
            const dist1BalanceFinal = await ethers.provider.getBalance(distributor1.address);
            const dist2BalanceFinal = await ethers.provider.getBalance(distributor2.address);
            const dist3BalanceFinal = await ethers.provider.getBalance(distributor3.address);
            ...
            expect(dist1Ganancia).to.be.gt(0n, "El distribuidor 1 debería haber recibido ETH");
            expect(dist2Ganancia).to.be.gt(0n, "El distribuidor 2 debería haber recibido ETH");
            expect(dist3Ganancia).to.be.gt(0n, "El distribuidor 3 debería haber recibido ETH");
